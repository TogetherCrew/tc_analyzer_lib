...
class TestNetworkGraphSixMonthRunOnceAvailableAnalytics(IsolatedAsyncioTestCase):
    async def test_networkgraph_six_months_period_run_once_available_analytics(self):
        """
        ...
        # filling up to 2 days ago with 170 documents
        memberactivity_data = create_empty_memberactivities_data(
            datetime.now() - timedelta(days=174), count=170
        )
        ...
        heatmaps_data = create_empty_heatmaps_data(
            datetime.now() - timedelta(days=180), count=176
        )
        ...
        #     f"""
        #     MATCH
        #         (g:{platform_label} {{id: '{platform_id}'}})
